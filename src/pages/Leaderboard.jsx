  ...
  const handleConnectBuddy = async () => {
    if (selectedBuddy && user) {
      try {
        ...
        if (!error) {
          // Update selected buddy with connected status immediately
          setSelectedBuddy(prev => ({ ...prev, connectionStatus: "connected" }))
          
          setAlertMessage(`✅ Connected with ${selectedBuddy.username}! 🎉`)
          setShowAlert(true)
          setTimeout(() => setShowAlert(false), 3000)
          
          // Refresh leaderboard to update connection status
          ...
          // Refresh leaderboard even on row-level security error
          await fetchLeaderboard()
        ...
        if (error.message?.includes("row-level security")) {
          ...
          // Refresh leaderboard even on error
          await fetchLeaderboard()
